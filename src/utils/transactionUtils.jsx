...
import { TransactionTableStyles } from '../styles/TransactionTableStyles'; // Import styles from the style file
...
export const generateTableRow = (item, index, formatTimestamp) => {
  ...
  return (
    <tr key={index} className={TransactionTableStyles.row}>
      ...
      <td className={TransactionTableStyles.cell}>
        <div className={TransactionTableStyles.hashText} title={item.transactionHash}>
          {item.transactionHash ? `${item.transactionHash.substring(0, 5)}...` : 'N/A'}
        </div>
      </td>
      <td className={TransactionTableStyles.cell}>
        <div className={TransactionTableStyles.hashText} title={item.from}>
          {item.from ? `${item.from.substring(0, 6)}...${item.from.slice(-4)}` : 'N/A'}
        </div>
      </td>
      <td className={TransactionTableStyles.cell}>
        <div className={TransactionTableStyles.hashText} title={item.to}>
          {item.to ? `${item.to.substring(0, 6)}...${item.to.slice(-4)}` : 'N/A'}
        </div>
      </td>
      <td className={TransactionTableStyles.cell}>
        <div className={TransactionTableStyles.valueText}>{tokenValue}</div>
      </td>
      <td className={TransactionTableStyles.cell}>
        <div className={TransactionTableStyles.valueText}>{item.type}</div>
      </td>
      <td className={TransactionTableStyles.cell}>
        <div className={TransactionTableStyles.tokenInfoContainer}>
          <div className={TransactionTableStyles.tokenName}>{item.tokenInfo.name}</div>
          <div className={TransactionTableStyles.tokenSymbol}>{item.tokenInfo.symbol}</div>
        </div>
      </td>
    </tr>
  );
